...
export default function LegalResearch() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.summary) {
        // Create a result object from the summary response
        setResults([{
          title: searchQuery,
          summary: data.summary,
          court: 'Legal Database',
          date: new Date().toLocaleDateString(),
          tags: ['Research', 'Query Result'],
          relevance: '100%'
        }]);
      } else {
        setResults([]);
      ...
      setError("Failed to perform legal research. Please check if the server is running on localhost:8000.");
    } finally {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-neutral-900">Legal Research</h1>
          </div>
          <p className="text-neutral-500">Search through thousands of Pakistani legal precedents, statutes, and case laws.</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="card p-6 bg-neutral-900 text-white">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-bold uppercase tracking-widest">Research Tip</h3>
            </div>
            <p className="text-xs text-neutral-400 leading-relaxed">
              Enter your query to search the legal database. Make sure the server at localhost:8000 is running.
            </p>
          </div>
        </div>
          ...
          {isSearching ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-neutral-500 max-w-md">
                Enter a query above to search the legal database.
              </p>
            </div>
          )}
        </div>
