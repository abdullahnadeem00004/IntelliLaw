...
import { Scale, ArrowLeft, Mail, Lock, AlertCircle, Loader2, User } from 'lucide-react';
import { useNavigate, useParams } from 'react-router-dom';
import { authAPI, LoginCredentials, SignupCredentials } from '../services/authService';
import { useAuth } from '../components/FirebaseProvider';

type UserRole = 'FIRM' | 'LAWYER' | 'CLIENT';

const roleConfig = {
  FIRM: { title: 'Law Firm Sign In', subtitle: 'Access your firm dashboard' },
  LAWYER: { title: 'Lawyer Sign In', subtitle: 'Manage your cases' },
  CLIENT: { title: 'Client Sign In', subtitle: 'Access your cases' },
};
...
export default function RoleLogin() {
  ...
  useEffect(() => {
    if (!role || !['FIRM', 'LAWYER', 'CLIENT'].includes(role)) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const credentials: LoginCredentials = { email, password, role: role as UserRole };
      ...
      <div className="max-w-md w-full space-y-8 animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="card p-8 shadow-xl shadow-neutral-200/50">
          <div className="text-center mb-8">
            <h2 className="text-xl font-bold text-neutral-900">{isSignUp ? `Create ${role} Account` : config.title}</h2>
        ...
        <p className="text-center text-xs text-neutral-400">
          ...
          <button className="underline hover:text-neutral-600">Terms of Service</button> and{' '}
          <button className="underline hover:text-neutral-600">Privacy Policy</button>.
        </p>
      </div>
    </div>
  );
